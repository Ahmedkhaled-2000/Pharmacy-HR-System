...
-- ==============================================================================
-- 15. منظومة إدارة ومزامنة الطلبات المتقدمة فائقة الخفة (Ultra-Low Quota Requests & Delta Sync)
-- ==============================================================================

-- 15.1 جدول الطلبات العلائقي المركزي المنفصل
...
-- 15.2 جدول تاريخ وتتبع حركات وحالات الطلب (Audit Trail & State Machine)
CREATE TABLE IF NOT EXISTS public.request_status_history (
    id BIGSERIAL PRIMARY KEY,
    request_id VARCHAR(36) NOT NULL REFERENCES public.requests(id) ON DELETE CASCADE,
...
-- 15.5 صندوق استلام السيرفر لمنع التكرار (Server Inbox & Deduplication)
...
-- 15.6 جدول حالة ومؤشر مزامنة الأجهزة (Sync State per Device)
...
-- 15.7 طابور العمليات المتعثرة (Dead Letter Queue)
...
-- 15.8 تريجر تسجيل التغييرات التلقائي في change_log
CREATE OR REPLACE FUNCTION public.fn_requests_change_tracker()
RETURNS TRIGGER AS $$
DECLARE
    op VARCHAR(20);
    rec RECORD;
BEGIN
    ...
    INSERT INTO public.change_log (
        entity_type, entity_id, branch_id, operation, delta_payload, timestamp
    ) VALUES (
        'request',
        rec.id,
        rec.branch_id,
...
FOR EACH ROW EXECUTE FUNCTION public.fn_requests_change_tracker();

-- 15.9 دالة ترحيل الطلبات السابقة من app_settings دون فقدان أي بيانات (Zero Data Loss)
...
-- 15.10 تفعيل RLS على الجداول الجديدة
